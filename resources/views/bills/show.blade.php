...
<div class="px-4 py-5 sm:p-6">
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Vendor: {{ $bill->vendor->name ?? 'N/A' }}</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('bills.index') }}" class="bg-white text-gray-700 px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                Back to List
            </a>
        </div>
    </div>
        ...
        <dl class="divide-y divide-gray-200">
            <div class="px-6 py-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 font-mono">{{ $bill->bill_number }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">Vendor</dt>
                    <dd class="mt-1 text-sm text-gray-900">
                        <a href="{{ route('vendors.show', $bill->vendor_id) }}" class="text-indigo-600 hover:text-indigo-900">
                ...
                <div>
                    ...
                    <dd class="mt-1 text-sm font-mono text-gray-900">SAR {{ number_format($bill->subtotal, 2) }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">VAT Amount</dt>
                    <dd class="mt-1 text-sm font-mono text-gray-900">SAR {{ number_format($bill->vat_amount, 2) }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">Total</dt>
                    <dd class="mt-1 text-lg font-bold font-mono text-gray-900">SAR {{ number_format($bill->total, 2) }}</dd>
                </div>
                @if($bill->amount_paid)
                <div>
                    <dt class="text-sm font-medium text-gray-500">Amount Paid</dt>
                    <dd class="mt-1 text-sm font-mono text-gray-900">SAR {{ number_format($bill->amount_paid, 2) }}</dd>
                </div>
                @endif
                @if($bill->balance_due)
                <div>
                    <dt class="text-sm font-medium text-gray-500">Balance Due</dt>
                    <dd class="mt-1 text-sm font-mono text-red-600">SAR {{ number_format($bill->balance_due, 2) }}</dd>
                </div>
                @endif
                @if($bill->notes)
                <div class="sm:col-span-2">
                    <dt class="text-sm font-medium text-gray-500">Notes</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $bill->notes }}</dd>
                </div>
                @endif
                <div>
                    <dt class="text-sm font-medium text-gray-500">Created By</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $bill->createdBy->name ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">Created At</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $bill->created_at->format('M d, Y H:i') }}</dd>
                </div>
                <div class="sm:col-span-2">
                    <dt class="text-sm font-medium text-gray-500">Attachments (Links)</dt>
                    <dd class="mt-1 text-sm text-gray-900">
                        @if($bill->attachments)
                            @foreach(array_filter(explode("\n", $bill->attachments)) as $link)
